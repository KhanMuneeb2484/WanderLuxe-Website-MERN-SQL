...
const Packages = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log(data)
        console.log(data.adminPackages)
        setPackages(data.adminPackages); // Store packages data
        console.log('this is ',packages); // Logs the packages data when component mounts or updates
  ...
  // Fix 1: Updated handleHotelSelection to ensure num_rooms is never undefined or invalid
  const handleHotelSelection = (cityId, hotelId, numRooms) => {
    // Ensure numRooms is at least 1 (default) if invalid value is passed
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage("Failed to create package. Please try again.");
      }
    } catch (error) {
            ...
            <div key={packageData.package_id} className="col-md-4">
              <div className="card mb-4">
                ...
                <div className="card-body">
                  <h5 className="card-title">{packageData.country_name}</h5>
                  <h6 className="card-subtitle mb-2 text-muted">
                    Guide: {packageData.guide_name}
                  </h6>
                  <p className="card-text">
                    Price: ${packageData.total_price} <br />
                    Duration: {packageData.total_days_stayed} days <br />
...
{Array.isArray(packageData.cities) && packageData.cities.length > 0 ? (
  <ul>
    {packageData.cities.map((city) => (
      <li key={city.package_city_id}>
        {city.city_name} ({city.days_stayed} days)
        <ul>
          {Array.isArray(city.locations) && city.locations.length > 0 ? (
  ...
  <p>No cities available</p>
                  ...
                  <Link to={`/packages/${packageData.package_id}`}>
                    <button className="btn btn-info mt-2">View More</button>
                  </Link>
      ...
      <form className="container mt-4 mb-8" onSubmit={handleSubmit}>
        ...
        {selectedCities.map((cityId) => (
          <div key={cityId} className="mt-3">
            <h5>
              City: {cities.find((city) => city.city_id === cityId)?.city_name}
            </h5>
            {/* Days Stayed */}
            <div className="mb-3">
            ...
            {locations[cityId]?.map((location) => (
              <div key={location.location_id} className="form-check">
                <input
                  ...
                  checked={
                    selectedLocations[cityId]?.includes(location.location_id) ||
                    false
                  }
                  onChange={(e) =>
            ...
            {hotels[cityId]?.map((hotel) => (
              <div key={hotel.hotel_id} className="form-check mb-3">
                <div className="d-flex align-items-center">
                  ...
                  >
                    {hotel.hotel_name} - ${hotel.price}/night
                  </label>
                </div>
